...
// LAUNCHED — waitlist mode is now OFF, so the full site is live. Set back to
// `!import.meta.env.DEV` (or `true`) to re-enable the pre-launch waitlist gate.
...
export default function Home() {
  ...
  // The Turnstile script is a single global: if it's blocked, every modal on this
  // page is blocked, so one flag covers contact, waitlist and newsletter.
  ...
  // Any token means the script came back (e.g. the visitor turned off their blocker).
  useEffect(() => {
    if (modalToken || waitlistToken || newsletterToken) setTurnstileBlocked(false)
  }, [modalToken, waitlistToken, newsletterToken])
  ...
  const handleSearchSelect = (item) => {
    if (item.comingSoon) { closeSearch(); return }
    closeSearch()
            ...
            <ul className="search-palette__results">
              {searchResults.length === 0
                ...
                : searchResults.map(item => (
                  <li key={item.label}>
                    <button
                      className={`search-palette__result${item.comingSoon ? ' search-palette__result--soon' : ''}`}
                    ...
                    >
                      ...
                      <span className={`search-palette__result-type search-palette__result-type--${item.comingSoon ? 'soon' : item.type}`}>
                        {item.comingSoon ? t.editorialComingSoon : item.type}
                      </span>
                    </button>
                  </li>
                ))
        ...
        <div className="modal__box" role="dialog" aria-modal="true" aria-labelledby="modal-title" ref={modalRef} onKeyDown={handleModalKeyDown}>
          ...
          {!modalSent ? (
            <>
              ...
              {turnstileBlocked && <p role="alert" className="form-turnstile-error">{t.turnstileError}</p>}
              <Turnstile onToken={setModalToken} onError={() => setTurnstileBlocked(true)} resetRef={modalTurnstileReset} className="modal__turnstile" />
              <div className="modal__footer">
                <button className="modal__btn" disabled={modalLoading || turnstileBlocked || !modalEmail.trim() || !modalMessage.trim() || (TURNSTILE_ENABLED && !modalToken)} onClick={handleModalSubmit}>
        ...
        <div className="modal__box" role="dialog" aria-modal="true" aria-labelledby="waitlist-title" ref={waitlistRef} onKeyDown={handleWaitlistKeyDown}>
          ...
          {!waitlistSent ? (
            <>
              ...
              {turnstileBlocked && <p role="alert" className="form-turnstile-error">{t.turnstileError}</p>}
              <Turnstile onToken={setWaitlistToken} onError={() => setTurnstileBlocked(true)} resetRef={waitlistTurnstileReset} className="modal__turnstile" />
              <div className="modal__footer">
                <button className="modal__btn" disabled={waitlistLoading || turnstileBlocked || !waitlistName.trim() || !waitlistEmail.trim() || (TURNSTILE_ENABLED && !waitlistToken)} onClick={handleWaitlistSubmit}>
        ...
        <div className="modal__box" role="dialog" aria-modal="true" aria-labelledby="newsletter-title" ref={newsletterRef} onKeyDown={handleNewsletterKeyDown}>
          ...
          {!newsletterSent ? (
            <>
              ...
              {turnstileBlocked && <p role="alert" className="form-turnstile-error">{t.turnstileError}</p>}
              <Turnstile onToken={setNewsletterToken} onError={() => setTurnstileBlocked(true)} resetRef={newsletterTurnstileReset} className="modal__turnstile" />
              <div className="modal__footer">
                <button className="modal__btn" disabled={newsletterLoading || turnstileBlocked || !newsletterEmail.trim() || (TURNSTILE_ENABLED && !newsletterToken)} onClick={handleNewsletterSubmit}>
